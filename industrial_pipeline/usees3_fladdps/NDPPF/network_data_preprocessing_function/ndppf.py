...
def get_ntp_server_time():
	try:
		...
		response = ntp_client.request('ntp.cnam.fr', version=3, timeout=2)
		return response.tx_timestamp
	except Exception as e:
		print('NTP failed, falling back to local time:', str(e))
		return time.time()
...
def consume_batches_preprocess(resource_name, initial_batch_size, consumer_topic, producer_topic_training, producer_topic_inference, iNDBF, eNDBF_):
	...
	for message in consumer:
		...
		if message_count == batch_size:
			...
			training_topics = ["preprocessed_cpu_topic_1", "preprocessed_cpu_topic_2", "preprocessed_cpu_topic_3"]
