...
class FLServer(FederatedLearningServicer):
    ...
    def average_fd_model_to_num_of_clients(self, sampled_client_dic_keys, resource_name):
        if self.model_type.name.lower() == 'torch':
            ...
            if self.aggregation_method.lower() == "fedprox":
                return self.fedprox_average_keras_models(sampled_client_dic_keys, resource_name)
            
            elif self.aggregation_method.lower() == "scaffold":
                if not hasattr(self, "global_control_variate"):
                    self.global_control_variate = [np.zeros_like(w) for w in self.global_model.get_weights()]
                    self.client_control_variates = {}

                # Collect client weights and control variates
                client_weights = {
                    cid: self.client_with_local_model_copy[resource_name][cid].model
                    for cid in sampled_client_dic_keys
                }

                for cid in sampled_client_dic_keys:
                    if cid not in self.client_control_variates:
                        self.client_control_variates[cid] = [np.zeros_like(w) for w in self.global_control_variate]

                updated_weights, new_global_cv = self.scaffold_average_keras_models(
                    client_weights,
                    self.global_control_variate,
                    self.client_control_variates,
                    resource_name
                )
                self.global_control_variate = new_global_cv
    ...
    def fedprox_average_keras_models(self, sampled_client_keys, resource_name, mu=0.01):
        local_params = []
        ...
        new_weights = []
        for w_avg, w_glob in zip(avg_model, global_weights):
            prox_weight = w_avg - mu * (w_avg - w_glob)
            new_weights.append(prox_weight)
        return new_weights

    def scaffold_average_keras_models(self, client_weights, global_control_variate, client_control_variates, resource_name):
        num_clients = len(client_weights)
        weight_shapes = [w.shape for w in list(client_weights.values())[0]]

        # Step 1: Aggregate model weights normally (FedAvg)
        averaged_weights = []
        for i in range(len(weight_shapes)):
            layer_weights = [client_weights[client][i] for client in client_weights]
            averaged_weights.append(sum(layer_weights) / num_clients)

        # Step 2: Update global control variate (c)
        new_global_cv = []
        for i in range(len(weight_shapes)):
            delta_c = sum(
                client_control_variates[client][i] - global_control_variate[i]
                for client in client_weights
            ) / num_clients
            new_global_cv.append(global_control_variate[i] + delta_c)

        return averaged_weights, new_global_cv
