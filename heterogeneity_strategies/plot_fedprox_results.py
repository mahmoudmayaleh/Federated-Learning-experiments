...
for alpha in alpha_values:
    filename = os.path.join(results_dir, f"server_history_FedProx{alpha}.json")
...
plt.title("FedProx Training Loss per Round")
plt.xlabel("Round")
plt.ylabel("Loss")
plt.legend()
plt.grid()
plt.tight_layout()
plt.savefig(os.path.join(figures_dir, "fedprox_loss_curves.png"))
plt.show()

plt.figure(figsize=(10, 5))
for alpha, accs in acc_curves.items():
    plt.plot(accs, label=f"α={alpha}")
plt.title("FedProx Test Accuracy per Round")
plt.xlabel("Round")
plt.ylabel("Accuracy")
plt.legend()
plt.grid()
plt.tight_layout()
plt.savefig(os.path.join(figures_dir, "fedprox_accuracy_curves.png"))
plt.show()
