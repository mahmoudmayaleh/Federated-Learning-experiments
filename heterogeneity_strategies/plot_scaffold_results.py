...
alpha_values = [0.1, 1, 10]  # Update if you used different alpha values for Scaffold

loss_curves = {}
acc_curves = {}

for alpha in alpha_values:
    filename = os.path.join(results_dir, f"server_history_Scaffold{alpha}.json")
...
plt.title("SCAFFOLD Training Loss per Round")
plt.xlabel("Round")
plt.ylabel("Loss")
plt.legend()
plt.grid()
plt.tight_layout()
plt.savefig(os.path.join(figures_dir, "scaffold_loss_curves.png"))
plt.show()

plt.figure(figsize=(10, 5))
for alpha, accs in acc_curves.items():
    plt.plot(accs, label=f"α={alpha}")
plt.title("SCAFFOLD Test Accuracy per Round")
plt.xlabel("Round")
plt.ylabel("Accuracy")
plt.legend()
plt.grid()
plt.tight_layout()
plt.savefig(os.path.join(figures_dir, "scaffold_accuracy_curves.png"))
plt.show()
